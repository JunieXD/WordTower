...
from app.api.api_responses import success_response, not_found_response
...
upgrade_hp_hp = settings.UPGRADE_HP_HP
upgrade_attack_attack = settings.UPGRADE_ATTACK_ATTACK
upgrade_crit_rate_crit_rate = settings.UPGRADE_CRIT_RATE_CRIT_RATE

router = APIRouter(prefix="/api/upgrade", tags=["upgrade"])

@router.post("/upgrade_hp")
async def upgrade_hp(session: SessionDep, user: User = Depends(get_current_user)):
    if user.coins < upgrade_hp_coins:
        return not_found_response(message="金币不足")
    user_upgrade_hp(session, user.username, upgrade_hp_coins, upgrade_hp_hp)
    return success_response(message="升级成功")

@router.post("/upgrade_attack")
async def upgrade_attack(session: SessionDep, user: User = Depends(get_current_user)):
    if user.coins < upgrade_attack_coins:
        return not_found_response(message="金币不足")
    user_upgrade_attack(session, user.username, upgrade_attack_coins, upgrade_attack_attack)
    return success_response(message="升级成功")

@router.post("/upgrade_crit_rate")
async def upgrade_crit_rate(session: SessionDep, user: User = Depends(get_current_user)):
    if user.coins < upgrade_crit_rate_coins:
        return not_found_response(message="金币不足")
    user_upgrade_crit_rate(session, user.username, upgrade_crit_rate_coins, upgrade_crit_rate_crit_rate)
    return success_response(message="升级成功")
